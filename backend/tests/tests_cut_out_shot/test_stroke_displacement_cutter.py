...
class TestCutOutShot:
    ...
    def test_normal_1(self, rawdata_df, stroke_displacement_sensors):
        """正常系：start_displacememt: 47.0, end_displacememt: 34.0。
        全13サンプル中8サンプルが切り出される。
        """

        stroke_displacement_cutter = StrokeDisplacementCutter(
            start_stroke_displacement=47.0,
            end_stroke_displacement=34.0,
            margin=0.1,
            sensors=stroke_displacement_sensors,
        )

        stroke_displacement_cutter.cut_out_shot(rawdata_df)
        actual: List[dict] = stroke_displacement_cutter.cut_out_targets
    ...
    def test_normal_2(self, rawdata_df, stroke_displacement_sensors):
        """正常系：start_displacememt: 46.9, end_displacememt: 34.0。
        全13サンプル中2サンプルが切り出される。
        """

        stroke_displacement_cutter = StrokeDisplacementCutter(
            start_stroke_displacement=46.9,
            end_stroke_displacement=34.0,
            margin=0.1,
            sensors=stroke_displacement_sensors,
        )
