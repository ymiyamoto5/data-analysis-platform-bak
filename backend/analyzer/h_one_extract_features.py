...
def narrowing_v4min_mab(shot_df, disp_narrowing=False, shot=9999):
    ...
    if disp_narrowing == True:
        ...
        ax3.axvspan(ma_min - 20, ma_min + 20, color="g", alpha=0.2)
        ax1.axvspan(var_start, var_end, color="g", alpha=0.2)
        ax3.axvline(var_end, color="r", alpha=0.5)
        ax4.axvline(ma_min, color="r", alpha=0.5)
...
def narrowing_v4min_ma(shot_df, disp_narrowing=False, shot=9999):
    ...
    if disp_narrowing == True:
        ...
        df[["v15ma"]].plot(ax=ax4)
        ax1.axvspan(var_start, var_end, color="g", alpha=0.2)
        ax3.axvline(df["v15"].idxmin(), color="r", alpha=0.5)
        ax4.axvline(df["v15ma"].idxmin(), color="r", alpha=0.5)
        #         plt.xlim([var_start-1000,var_end+1500]);
        plt.xlim([var_start - 500, var_end + 1000])
        plt.suptitle("15 points velocity / shifted vrms shot:%d" % shot)
        plt.show()
...
def narrowing_v4min_vrms(shot_df, disp_narrowing=False, shot=9999):
    ...
    df["v15_04"] = df["load04"].rolling(RW, center=False).mean().diff()
    df["v15"] = df["v15_01"] + df["v15_02"] + df["v15_03"] + df["v15_04"]
    df["vrms"] = (df["v15"].rolling(20, center=True).apply(_rms).shift(55) + 0.5) ** 2
    ...
    if disp_narrowing == True:
        ...
        ax1.axvspan(var_start, var_end, color="g", alpha=0.2)
        ax3.axvline(df["v15"].idxmin(), color="g", alpha=0.5)
        plt.xlim([var_start - 1000, var_end + 1500])
        plt.suptitle("15 points velocity / shifted vrms shot:%d" % shot)
        plt.show()
...
def narrowing_varch_v4min(shot_df, disp_narrowing=False, shot=9999):
    ...
    if disp_narrowing == True:
        ...
        df[["svar"]].plot(ax=ax3)
        ax3.axvspan(s_start, s_end, color="r", alpha=0.2)
        ax3.axhline(SVAR_CRITERIA, color="g", alpha=0.5)
        ...
        df[["v15"]].plot(ax=ax6)
        ax4.axvspan(var_start, var_end, color="g", alpha=0.2)
        ax6.axvline(df["v15"].idxmin(), color="g", alpha=0.5)
        ax1.axvspan(var_start, var_end, color="g", alpha=0.2)
...
def narrowing_var_ch(shot_df, disp_narrowing=False, shot=9999):
    ...
    if disp_narrowing == True:
        ...
        ax3 = plt.subplot(313, sharex=ax1)
        df[["svar"]].plot(ax=ax3)
        ax3.axvspan(var_start, var_end, color="r", alpha=0.2)
        ax3.axhline(SVAR_CRITERIA, color="g", alpha=0.5)
...
def breaking_vmin_amin(
    d, spm, fs=100000, low=0, high=8000, r_window=1, Debug=False, shot=9999, ch="loadxx", debug_xlim=[-50, 50]
):
    """破断点
    ...
    if Debug is True:
        ...
        df.o.plot(ax=ax[0], alpha=0.3, color="black")
        ax[0].axvline(h, color="r")
        ax[2].axvline(vmin, color="r")
        ax[3].axvline(h, color="r")
        ax[3].axvspan(NARROW_PADDING, vmin, color="g", alpha=0.2)
...
def breaking_var_vrms(
    d, spm, fs=100000, low=0, high=8000, r_window=1, Debug=False, shot=9999, ch="loadxx", debug_xlim=[-100, 100]
):
    """破断点
    ...
    if Debug is True:
        ax = df[["d", "var29", "var29_v", "vrms", "v", "a"]].plot(
            figsize=(10, 8),
            subplots=True,
            color="b",
            title="%s shot:%d ch:%s" % (sys._getframe().f_code.co_name, shot, ch),
        ...
        ax[5].axvspan(varmax, rmsmax, color="g", alpha=0.3)
    ...
    return (
        ...
        df.d[h],
        [h - varmax, rmsmax - h],
    )  # dfは50:-50の範囲でreset_indexされているので、df.dのindexはh+50である必要は無い


def breaking_rmean_dmin(
    d, spm, fs=100000, low=0, high=8000, r_window=1, Debug=False, shot=9999, ch="loadxx", debug_xlim=[-100, 100]
):
    """破断点
    ...
    if Debug is True:
        ax = df[["m", "o_m", "d", "v", "a"]].plot(
            figsize=(10, 8),
            subplots=True,
            color="b",
            title="%s shot:%d ch:%s" % (sys._getframe().f_code.co_name, shot, ch),
        )
        df.o.plot(ax=ax[0], alpha=0.3, color="black", label="o")
        plt.legend()
        ax[0].axvline(h, color="r")
        ax[1].axvline(break_through, color="r")
        ax[4].axvline(h, color="r")
        ax[4].axvspan(break_through - 20, break_through, color="g", alpha=0.3)
        plt.xlim(h + debug_xlim[0], h + debug_xlim[1])
...
def breaking_varmax29idiff(
    ...
    df["v"] = df.o.diff()  # 速度
    df["a"] = df.v.diff()  # 加速度
    df["v9"] = df.d.rolling(9, center=True, min_periods=1).var() * -df.d.rolling(9, center=True, min_periods=1).apply(
        _idiff
    )
    df["v29"] = df.d.rolling(29, center=True, min_periods=1).var() * -df.d.rolling(
        29, center=True, min_periods=1
    ).apply(_idiff)
    ...
    if Debug is True:
        ...
        df.o.plot(ax=ax[0], alpha=0.3, color="black")
        ax[0].axvline(h, color="r")
        # ax[2].axhline(criteria,color='g')
        ax[2].axvline(varmax29idiff, color="r")
        # ax[2].axhline(criteria,color='g')
        ax[3].axvline(varmax9idiff, color="r")
        # ax[2].axhline(criteria,color='g')
...
def load_start2(d, spm, r_window=399, Debug=False, shot=999, ch="loadxx", debug_xlim=[-1000, 1500]):
    ...
    if Debug is True:
        ...
        ax[1].axvspan(sd_start, sd_end, color="g", alpha=0.3), ax[1].axhline(0.2, color="g")
        ax[3].set_ylim(-0.1, 1.1)
        ax[3].axhline(0.2, color="g")
        plt.xlim(h + debug_xlim[0], h + debug_xlim[1])
...
def load_start3(d, spm, r_window=399, Debug=False, shot=999, ch="loadxx", debug_xlim=[-1000, 1500]):
    ...
    if Debug is True:
        ax = df[["d", "sd", "v", "sv", "a"]].plot(
            figsize=(10, 8), subplots=True, title="%s shot:%d,ch=%s" % (sys._getframe().f_code.co_name, shot, ch)
        )
        df.o.plot(ax=ax[0], alpha=0.3, color="black")
        ax[0].axvline(h, color="r")
        ax[1].set_ylim(-0.1, 1.1)
        ax[1].axhline(0.2, color="g")
...
def max_load(
    d, spm, fs=100000, low=0, high=None, r_window=19, Debug=False, shot=9999, ch="loadxx", debug_xlim=[-1500, 1000]
):
    """最大荷重点
    ...
    **kwargs
    ...
    if narrowing is not None:
        ...
        if sub_func is not None:
            ...
            if (
                sub_start - NARROW_PADDING < 0
                or sub_end + NARROW_PADDING > shot_data.index[-1]
                or sub_end - sub_start < 10
            ):
    ...
    if disp_chart:
        ...
        for c in range(len(chs)):
            plt.plot(
                shot_data.index, shot_data[chs[c]], label=chs[c], alpha=0.3, color=cmap(c)
            )  # 「流れ」があるのでscatterよりlineの方が見やすいと思う
            # plt.scatter(shot_data.index,shot_data[chs[c]],label=chs[c],s=2,alpha=1.0,color=[cmap(c)]*len(shot_data))
            plt.scatter(
                [argmax[c]], [valmax[c]], marker="o", s=200, alpha=0.5, color=[cmap(c)]
            )  # plotとscatterのcolor mapを揃える
...
if __name__ == "__main__":
    ...
    for f in range(0, len(flist), 50):
        ...
        argmax, valmax, debugval = extract_features(
            df1, 80.0, breaking_var_vrms, sub_func=narrowing_var_ch
        )  # SPMここでは固定
