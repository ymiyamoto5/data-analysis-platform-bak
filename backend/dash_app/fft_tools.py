...
try:
    # Jupyter/IPythonでインタラクティブにグラフ描画可能な環境でのみ実行。__IPYTHON__が定義されていない環境ではNameErrorが発生し以下は実行されない。
...
def find_peaks(a, amp_thre=None, local_width=1, min_peak_distance=1, num_peaks=5):
    ...
    for i in idxs_with_offset:
        is_local_maximum = a_extend[i] >= 0 and a_extend[i] >= np.max(a_extend[i - local_width : i + local_width + 1])  # 最大値を検出
        is_local_minimum = a_extend[i] < 0 and a_extend[i] <= np.min(a_extend[i - local_width : i + local_width + 1])  # 最小値を検出
...
# FFT、及びSTFTの関数定義
def fft_spectrum(x, fs, num_average=1, overlap=0.5, c=False, xrange=None, peak=False, amp_thre=None, min_peak_distance=1, num_peaks=5):
    """
